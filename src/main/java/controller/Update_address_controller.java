...
@WebServlet( "/update_address_controller" )
public class Update_address_controller extends HttpServlet{
			...
			throws ServletException , IOException
	{
		...
		try {
			...
			// 受け取ったメアドと新たな住所を引数に、データベース( LOGIN_USER )の住所を更新する
			int count = 0 ;
			
			count = Dao_user.update_address( address, email ) ;
			...
			if( count == 1 ) {
				...
				msg = "お客様住所更新に成功しました。ログインページより再度ログインをお願いします。" ;
			}
			else {
				msg = "お客様住所更新に失敗しました。ログインページより最初からやり直してください。" ;
			}
